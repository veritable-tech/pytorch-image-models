...
class PositionalEncodingFourier(nn.Module):
    ...
    def forward(self, B: int, H: int, W: int):
        ...
        weight_dtype = self.token_projection.weight.dtype
        y_embed = torch.arange(1, H+1, dtype=weight_dtype, device=device).unsqueeze(1).repeat(1, 1, W)
        x_embed = torch.arange(1, W+1, dtype=weight_dtype, device=device).repeat(1, H, 1)
        y_embed = y_embed / (y_embed[:, -1:, :] + self.eps) * self.scale
        x_embed = x_embed / (x_embed[:, :, -1:] + self.eps) * self.scale
        dim_t = torch.arange(self.hidden_dim, dtype=weight_dtype, device=device)
